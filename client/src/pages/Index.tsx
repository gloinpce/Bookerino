...
const Index = () => {
  ...
  // Hide duplicate text elements in bottom part of page
  React.useEffect(() => {
    const hideBottomDuplicates = () => {
      ...
      allElements.forEach((element) => {
        ...
        // Check if element is in bottom part of page (last 300px)
        const isInBottom = rect.bottom > viewportHeight - 300 || 
                          (rect.top > documentHeight - 300 && rect.top < documentHeight);
        
        // Check if element is in left side (left 500px)
        const isOnLeft = rect.left < 500;
        ...
        const hasDuplicateText = (
          ...
          (text.includes('Bookerino') && text.includes('Funcționalități') && text.includes('Prețuri') && text.includes('Despre') && text.includes('Înregistrare') && text.includes('Autentificare') && text.length < 600) ||
          (text.includes('După autentificare, puteți accesa') && !element.closest('[class*="Card"]')) ||
          (text.includes('Panoul de control') && text.includes('Rapoarte și analize') && text.includes('Setările contului') && text.includes('Gestionarea abonamentului'))
        );
        
        // Only hide if it's in bottom AND has duplicate text AND not in main footer/nav/section/card
        ...
        const isInMainContent = element.closest('[id="root"] > div > div > div');
        
        if (hasDuplicateText && isInBottom && !isInMainFooter && !isInNav && !isInSection && !isInCard) {
        ...
        } else if (hasDuplicateText && isOnLeft && isInBottom && !isInMainFooter && !isInNav) {
          // Also hide if it's on left side in bottom
          element.style.display = 'none';
          element.style.visibility = 'hidden';
          element.style.opacity = '0';
        }
    ...
    // Run after page loads and on scroll
    setTimeout(hideBottomDuplicates, 100);
    setTimeout(hideBottomDuplicates, 500);
    setTimeout(hideBottomDuplicates, 1000);
    setTimeout(hideBottomDuplicates, 2000);
    
    // Also run on scroll to catch dynamically positioned elements
    window.addEventListener('scroll', hideBottomDuplicates);
    window.addEventListener('resize', hideBottomDuplicates);
    
    return () => {
      window.removeEventListener('scroll', hideBottomDuplicates);
      window.removeEventListener('resize', hideBottomDuplicates);
    };
  }, []);
